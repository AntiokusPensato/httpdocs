...
if (isset($_GET['page_id']) && !$_POST) {
    // prepare SQL query
    $sql = 'SELECT page_id, title, created
    ...
    if ($stmt->prepare($sql)) {
        ...
        $stmt->bind_result($page_id, $title, $created);
        $stmt->fetch();
    }
}
// if confirm deletion button has been clicked, delete record
if (isset($_POST['delete'])) {
    $sql = 'DELETE FROM php_blog_page WHERE page_id = ?';
    if ($stmt->prepare($sql)) {
...
// cancel button clicked, or $_GET['page_id'] not defined
if ($deleted || isset($_POST['cancel_delete']) || !isset($_GET['page_id'])) {
...
<main>
    ...
    // IF no page_id value - Zero same as FALSE (== vs ===)
    if ($page_id == 0) { ?>
    ...
    <form id="form1" method="post">
        <fieldset>
            ...
            <ol>
                <li>
                    <?php if (isset($page_id) && $page_id > 0) { ?>
